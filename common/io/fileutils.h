...
*Last Modified: 2015年11月21日 星期六 17时49分47秒
*Purpose:
**/
...
public:
    ...
    static void save_container_file(const std::string& filename, const Container& container, FILEMODE file_mode = FILEMODE::OVERWRITE)
    {
        ofstream ofs;
        if (file_mode == FILEMODE::OVERWRITE) {
            ofs.open(filename);
    ...
    {
        // 调用bash命令
        std::string command("rm " + pattern + "2>/dev/null");
        return system(command.c_str());
    }
    // 重命名文件
