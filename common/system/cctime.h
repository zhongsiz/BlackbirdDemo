...
*Last Modified: 2015年10月28日 星期三 22时56分20秒
*Purpose: 日期类封装
**/
...
public:
    ...
    timewrapper();
    // 通过YYYY-MM-DD构造
    timewrapper(const std::string& date, time_type_enum time_type = time_type_enum::DATE);
public:
    // 判断日期YYYY-MM-DD是否相等
    ...
    // 判断小时是否相等
    bool is_hour_equal(const timewrapper& rsh) const {
        return timeinfo->tm_hour == rsh.timeinfo->tm_hour;
    }
public:
